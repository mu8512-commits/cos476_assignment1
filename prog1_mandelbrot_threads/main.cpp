...
void usage(const char* progname) {
    ...
    printf("  -i  --impl <INT>   Use specified mandelbrotThread settings\n");
...
int main(int argc, char** argv) {
    ...
    static struct option long_options[] = {
        {"threads", 1, 0, 't'},
        {"view", 1, 0, 'v'},
        {"implementation", 1, 0, 'i'},
        {"help", 0, 0, '?'},
        {0 ,0, 0, 0}
    };

    while ((opt = getopt_long(argc, argv, "t:v:i:?", long_options, NULL)) != EOF) {
    ...
    //
    if(implThread == 0 || implThread == 1) {
        double minThread = 1e30;
        for (int i = 0; i < 5; ++i) {
        memset(output_thread, 0, width * height * sizeof(int));
            double startTime = CycleTimer::currentSeconds();
            mandelbrotThread(implThread, numThreads, x0, y0, x1, y1, width, height, maxIterations, output_thread);
            double endTime = CycleTimer::currentSeconds();
        ...
        if (! verifyResult (output_serial, output_thread, width, height)) {
            ...
            printf ("PASS: Serial and threaded images match.\n");
        }
